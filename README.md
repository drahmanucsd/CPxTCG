...
- **Tunes** (`/tunes`): 20 public-domain standards, **iReal Pro import** (links, playlists, exported HTML), chart text, **scan a page** (local OCR → bars → editable chart → practice on your own photo with a moving cursor). Chart / flat form / guide-tone views, section loops, transposition, synthesized **rhythm section** (bass + drums in styles), and **backing tracks with nothing to do**: a curated catalog (tune → verified YouTube track + tempo + beat-1 offset) makes “Play with the track” play in sync automatically; tunes not in the catalog get a live search, and the first tap is remembered. **Records**: drop your own recording (or stems from a separator), mute the piano, play with the real thing sample-accurately.
...
Pending the YouTube key: filling `data/backing-catalog.json` (`scripts/curate-backing.ts` + one verification pass at a piano) and live search. Not yet: in-browser stem separation + beat tracking + form alignment (Phase 6b in the plan), Basic-Pitch (note-level mic), melody OMR, comping-rhythm trainer. Details and order in [`docs/04-build-plan.md`](docs/04-build-plan.md).
