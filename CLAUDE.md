...
- Rhythm placement is its own thing (`packages/theory/src/figures.ts`, `timing.ts`): a push played on the downbeat is a different rhythm, not a late one, and is graded after the constant offset is removed. Figure hits are `{ beat, slot }`, never beat fractions, so swing stays tempo-dependent.
- Melody grading against a written line is `packages/theory/src/compare.ts`, not `gradeMelody` (which ignores rhythm on purpose). It applies swing to the written line first, caps its tolerance below the gap between adjacent subdivisions, and searches for the performance's offset before grading it.
